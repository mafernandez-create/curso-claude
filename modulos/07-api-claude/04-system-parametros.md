...
- [ ] Entender por qué **temperature/top_p/top_k ya no se usan** en los modelos actuales.

> **⚠️ Actualización (2026):** el plan original listaba `temperature` entre los parámetros habituales. En los modelos actuales (Opus 4.7/4.8, Fable 5) **los parámetros de sampling se eliminaron** y enviar `temperature`, `top_p` o `top_k` **da error 400**. Esta lección refleja el estado vigente.
...
- **Enviar `temperature`/`top_p`:** error 400 en los modelos actuales. Elimínalos.
...
3. Los parámetros de sampling (temperature, top_p, top_k) se eliminaron: hoy se guía con prompting y effort.
